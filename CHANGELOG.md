...
- 🔁 **Crash-Proof Event Channel Routing Persistence** (`set-event-channel.ts`, `status.ts`, `index.ts`, `set-event-channel.test.ts`): Implemented a 4-tier persistent hierarchy (In-Memory cache → `EVENT_CHANNEL_*` / `WELCOME_CHANNEL_ID` environment variables → `bot_config` table → `LOG_CHANNEL_ID` / `member_join` fallbacks) with automatic startup hydration (`loadPersistedEventRoutes()`) on `ClientReady` and `/admin status` routing inspection, ensuring event routes (`member_join`, `member_leave`, `pledge_create`, `pledge_delete`, `pledge_upgrade`, `pledge_downgrade`) are 100% immune to Redis outages, network glitches, or container restarts
- 🛡️ **Transient TLS/SSL Handshake & Network Retry Resilience** (`errorHandler.ts`, `posts-publish.ts`, `posts-update.ts`, `patreonPoller.ts`, `logger.ts`): Implemented `sendChannelWithRetry` with exponential backoff to handle transient Cloudflare/Discord edge SSL handshake errors (`sslv3 alert handshake failure / SSL alert 40`), `ECONNRESET`, and 5xx errors without dropping post announcements or crashing alert dispatches
- 👋 **Tier-Aware Member Departure & Cancellation Alerts** (`embedBuilder.ts`, `members-delete.ts`, `members-pledge-delete.ts`): Added `createDepartureEmbed` to include the specific Patreon tier/pledge the member departed from (e.g. `**Mhee Wattson** has ended their **💎 Diamond** pledge.`), looking up previous tier state from DB mappings and webhook relationships
